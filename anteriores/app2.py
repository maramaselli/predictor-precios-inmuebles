...
def predict_price(rooms, bedrooms, bathrooms, surface_total, surface_covered, place_name, property_type, state_name):
    ...
    if place_name in localidad_a_zona:
        if localidad_a_zona[place_name] != state_name:
            return f"⚠️ La localidad {place_name} pertenece a la zona '{localidad_a_zona[place_name]}'. Por favor selecciona la zona correcta."
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        with gr.Column():
            gr.Examples(
                examples=[
                    [3, 2, 1, 60, 55, "Palermo", "Departamento", "Capital Federal"],
                    [4, 3, 2, 120, 110, "Belgrano", "Departamento", "Capital Federal"],
                    [2, 1, 1, 45, 40, "Recoleta", "Departamento", "Capital Federal"],
                    [3, 2, 2, 75, 70, "Caballito", "PH", "Capital Federal"],
                ],
                inputs=[rooms, bedrooms, bathrooms, surface_total, surface_covered, place_name, property_type, state_name],
                label="Ejemplos de CABA"
            )
        with gr.Column():
            gr.Examples(
                examples=[
                    [3, 2, 2, 80, 75, "Lomas de Zamora", "Casa", "Bs.As. G.B.A. Zona Sur"],
                    [2, 1, 1, 45, 40, "Morón", "PH", "Bs.As. G.B.A. Zona Oeste"],
                    [4, 3, 3, 140, 130, "San Isidro", "Casa", "Bs.As. G.B.A. Zona Norte"],
                    [3, 2, 1, 65, 60, "Quilmes", "Departamento", "Bs.As. G.B.A. Zona Sur"],
                ],
                inputs=[rooms, bedrooms, bathrooms, surface_total, surface_covered, place_name, property_type, state_name],
                label="Ejemplos de GBA"
            )
    # ====================================================
