...
# === FUNCIÓN DE PREDICCIÓN (output en Markdown, estilo minimalista) ===
def predict_price(rooms, bedrooms, bathrooms, surface_total, surface_covered, place_name, property_type, state_name):
    ...
    if surface_total < surface_covered:
        return "La superficie total no puede ser menor que la superficie cubierta."

    # Validación localidad-zona
    if place_name in localidad_a_zona:
        zona_correcta = localidad_a_zona[place_name]
        if zona_correcta != state_name:
            return f"La localidad **{place_name}** pertenece a **{zona_correcta}**. Seleccione la zona correspondiente."
    ...
    return f"**Precio estimado:** USD {pred:,.0f}"


# === AUTO-SELECCIÓN DE ZONA ===
def update_zone(place_name):
    return localidad_a_zona.get(place_name, None)


# === UI GRADIO ===
place_name_options = sorted(list(place_name_freq.keys()))
property_type_options = list(property_ohe.categories_[0])
state_name_options = list(state_ohe.categories_[0])

with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("## Estimador de Valor Inmobiliario")
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            # Auto-asigna zona al seleccionar localidad
            place_name.change(update_zone, inputs=place_name, outputs=state_name)

            predict_btn = gr.Button("Calcular valor")
            output = gr.Markdown()
